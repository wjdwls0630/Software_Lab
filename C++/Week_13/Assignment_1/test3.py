...
data_num = 0
with open("./data/Test3/x_n_1_MagPh.txt", 'r') as dataFile:
    while True:
...
with open("./data/Test3/X_k_1_MagPh.txt", 'r') as dataFile:
    while True:
        line = dataFile.readline()
        if not line:
            break
...
with open("./data/Test3/x_n_1_ReIm_test.txt", 'r') as dataFile:
    while True:
        line = dataFile.readline()
        if not line:
            break
...
ax.set(title="x[n]_IFFT")
ax.stem(n, x_n_mag_idft, use_line_collection=True)
ax.set_xlabel(r'$n$')
ax.set_ylabel(r'$x[n]$')
ax.grid()
pyplot.savefig("./data/Test3/Test3.png")
pyplot.show()
